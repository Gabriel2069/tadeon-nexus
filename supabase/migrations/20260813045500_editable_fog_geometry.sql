alter table public.tabletop_fog_strokes
  add column if not exists geometry text not null default 'brush';

alter table public.tabletop_fog_strokes
  drop constraint if exists tabletop_fog_strokes_geometry_check;

alter table public.tabletop_fog_strokes
  add constraint tabletop_fog_strokes_geometry_check
  check (geometry in ('brush', 'rectangle', 'ellipse', 'polygon'));

comment on column public.tabletop_fog_strokes.geometry is
  'Persistent editable geometry for fog operations rendered by the tabletop canvas.';

CREATE OR REPLACE FUNCTION private.save_tabletop_visibility_state(target_scene_id uuid, expected_visibility_version integer, illumination numeric, fog_enabled boolean, fog_opacity numeric, wall_documents jsonb, light_documents jsonb, fog_documents jsonb)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
