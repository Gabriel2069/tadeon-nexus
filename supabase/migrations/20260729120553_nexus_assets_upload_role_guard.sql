-- Keep read-only workspace viewers and campaign observers from reserving uploads.

CREATE OR REPLACE FUNCTION private.can_contribute_workspace(target_workspace_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION private.can_contribute_campaign(target_campaign_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
CREATE POLICY "Asset sessions: contributors reserve"
  ...
  WITH CHECK (
    ...
    AND (
      (SELECT private.can_contribute_workspace(workspace_id))
      OR (
        campaign_id IS NOT NULL
        AND (SELECT private.can_contribute_campaign(campaign_id))
      )
    )
  );
...
COMMENT ON FUNCTION private.can_contribute_workspace(uuid) IS
  'True for application admins, workspace owners, admins and members; false for viewers.';
COMMENT ON FUNCTION private.can_contribute_campaign(uuid) IS
  'True for application admins, workspace managers, masters, co-masters and players; false for observers.';
