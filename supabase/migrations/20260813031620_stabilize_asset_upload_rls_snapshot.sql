...
REVOKE ALL ON FUNCTION private.can_finalize_asset_catalog_row(
  uuid, uuid, uuid, public.asset_provider, text, text, text, text, bigint
) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION private.can_finalize_asset_catalog_row(
  uuid, uuid, uuid, public.asset_provider, text, text, text, text, bigint
...
WITH CHECK (
  ...
  AND (
    SELECT private.can_finalize_asset_catalog_row(
      assets.id,
      assets.workspace_id,
      assets.campaign_id,
      assets.provider,
