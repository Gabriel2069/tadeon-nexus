...
alter table public.tabletop_fog_strokes
  add column if not exists audience jsonb not null default '{"scope":"global"}'::jsonb;

alter table public.tabletop_fog_strokes
  drop constraint if exists tabletop_fog_strokes_audience_check;

alter table public.tabletop_fog_strokes
  add constraint tabletop_fog_strokes_audience_check
  check (
    jsonb_typeof(audience) = 'object'
    and coalesce(audience ->> 'scope', 'global') in ('global', 'users', 'roles')
    and (
      not (audience ? 'userIds')
      or jsonb_typeof(audience -> 'userIds') = 'array'
    )
    and (
      not (audience ? 'roles')
      or jsonb_typeof(audience -> 'roles') = 'array'
    )
  );

comment on column public.tabletop_fog_strokes.audience is
  'Audience filter for fog memory. scope=global, users (userIds UUID strings), or roles (participant role strings).';
...
begin
  if actor is null then
    raise insufficient_privilege using message = 'TABLETOP_AUTH_REQUIRED';
  end if;
  ...
  select scene.visibility_version
  ...
  if current_version is null then
    raise insufficient_privilege using message = 'TABLETOP_SCENE_FORBIDDEN';
  end if;

  if current_version <> expected_visibility_version then
    raise exception using
      errcode = 'P0001',
      message = 'TABLETOP_VISIBILITY_VERSION_CONFLICT';
  end if;
  ...
  if jsonb_typeof(wall_documents) <> 'array'
    or jsonb_array_length(wall_documents) > 512
    or jsonb_typeof(light_documents) <> 'array'
    or jsonb_array_length(light_documents) > 256
    or jsonb_typeof(fog_documents) <> 'array'
    or jsonb_array_length(fog_documents) > 512
  then
    raise data_exception using message = 'TABLETOP_INVALID_VISIBILITY';
  end if;

  select level.id
  into fallback_level_id
  ...
  if exists (
    ...
    lateral jsonb_array_elements(
      case
        when jsonb_typeof(fog_document -> 'points') = 'array'
          then fog_document -> 'points'
        else '[]'::jsonb
      end
    ) point
  ...
  if exists (
    select 1
    from jsonb_array_elements(fog_documents) fog_document
    where jsonb_typeof(coalesce(fog_document -> 'audience', '{"scope":"global"}'::jsonb)) <> 'object'
      or coalesce(fog_document -> 'audience' ->> 'scope', 'global') not in ('global', 'users', 'roles')
  ) then
    raise data_exception using message = 'TABLETOP_INVALID_FOG_AUDIENCE';
  end if;
  ...
  if exists (
    ...
    where light.entity_id is not null
      and not exists (
        ...
        where entity.id = light.entity_id
          and entity.scene_id = target_scene_id
      )
  ) then
  ...
  select
    ...
    light.enabled,
    light.casts_shadows,
    coalesce(light.properties, '{}'::jsonb),
    actor,
    actor
  ...
  select
    ...
    fog.operation,
    coalesce(fog.geometry, 'brush'),
    fog.points,
    fog.radius,
    fog.sequence_index,
    coalesce(fog.audience, '{"scope":"global"}'::jsonb),
    actor
