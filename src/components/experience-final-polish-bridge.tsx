      ...
      {skillHost && skillPopover && createPortal(
        <div className="tadeon-skill-modifier-bridge">
          <div className="tadeon-skill-modifier-tabs" role="tablist" aria-label={`Treino e modificadores de ${skillName}`}>
            ...
            <button type="button" role="tab" aria-selected={modifierTab === "modifiers"} onClick={() => setModifierTab("modifiers")}>
            ...
            <section className="tadeon-skill-modifier-panel">
              ...
              <label>
                ...
                <Input
                  ...
                  onChange={(event) => setManualModifiers((current) => ({ ...current, [skillName]: Number(event.target.value) || 0 }))}
                  onBlur={(event) => void persistManual(Math.max(-50, Math.min(50, Number(event.target.value) || 0)))}
                />
              ...
              <div className="tadeon-skill-modifier-passive">
                <span>Habilidades passivas</span><strong>{passive >= 0 ? "+" : ""}{passive}</strong>
                <small>Calculado automaticamente por habilidades que citam esta perícia e um bônus/penalidade.</small>
              </div>
