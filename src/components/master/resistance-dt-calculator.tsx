...
export function ResistanceDtCalculator({
  ...
  const guidance = RESISTANCE_GUIDANCE.find(
    (item) => item.value === resistance,
  );
    ...
    <Card className="tadeon-surface overflow-hidden rounded-2xl border-primary/35">
      ...
      {open && (
        <div id="resistance-dt-fields">
          <div
            className={`grid gap-3 p-4 ${compact ? "sm:grid-cols-2" : "sm:grid-cols-4"}`}
          >
            <label className="space-y-1.5">
              ...
              <Input
                ...
                value={attribute}
                onChange={(event) =>
                  setAttribute(Number(event.target.value) || 0)
                }
              />
            </label>
            <label className="space-y-1.5">
              ...
              <select
                value={grade}
                onChange={(event) =>
                  setGrade(event.target.value as ChannelingGrade)
                }
            ...
            <label className="space-y-1.5">
              ...
              <select
                value={intensity}
                onChange={(event) =>
                  setIntensity(event.target.value as ChannelingIntensity)
                }
                className="h-9 w-full rounded-md border border-border bg-input px-2 text-sm"
              >
          ...
          <div className="border-t border-border/60 p-4">
            ...
            <p className="mt-3 text-[10px] text-muted-foreground">
              Fórmula: 10 + Atributo + Potência do Grau + Intensidade +
              modificador.
            </p>
          </div>
        </div>
