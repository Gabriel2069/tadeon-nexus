...
export function TabletopFinalInteractionBridge() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const entity = snapshot?.selectedIds.length === 1 ? snapshot.scene.entities.find((candidate) => candidate.id === snapshot.selectedIds[0]) ?? null : null;
  ...
  const refreshItems = useCallback(async () => {
    ...
    const entities: RenameItem[] = snapshot.scene.entities.map((entity) => ({ id: entity.id, kind: "entity", label: entity.label || "Entidade sem nome" }));
    ...
    const wallItems = ((walls.data ?? []) as Array<{ id: string; label: string | null; wall_type: string }>).map((row) => ({ id: row.id, kind: "wall" as const, label: row.label?.trim() || row.wall_type.replaceAll("_", " ") }));
    const lightItems = ((lights.data ?? []) as Array<{ id: string; label: string | null }>).map((row, index) => ({ id: row.id, kind: "light" as const, label: row.label?.trim() || `Luz ${index + 1}` }));
    const fogItems = ((fog.data ?? []) as Array<{ id: string; label: string | null; operation: string }>).map((row, index) => ({ id: row.id, kind: "fog" as const, label: row.label?.trim() || `${row.operation === "reveal" ? "Revelação" : "Cobertura"} ${index + 1}` }));
  ...
  useEffect(() => { if (open) void refreshItems(); }, [open, refreshItems]);
  const selected = useMemo(() => items.find((item) => `${item.kind}:${item.id}` === selectedKey) ?? null, [items, selectedKey]);
  useEffect(() => { setDraft(selected?.label ?? ""); }, [selected]);
  ...
  return createPortal(
    <>
      <Button size="sm" variant="ghost" className="tadeon-rename-launcher" onClick={() => setOpen(true)} title="Renomear qualquer elemento da cena"><Pencil /> Renomear</Button>
      {open && (
