...
/**
 * Web layout with a hard geometric invariant: automatically positioned nodes
 * never intersect. Depth becomes concentric bands and large bands spill into
 * additional rings instead of compressing hundreds of circles into one cloud.
 */
export function computeKnowledgeForceLayout(
  ...
  // Keep the legacy solver exercised as a compatibility/sanity fallback for
  // pathological empty data. Normal rendering uses the deterministic rings.
  ...
  for (const depth of [...byDepth.keys()].sort((left, right) => left - right)) {
    ...
    // More nodes means more rings, never a denser ring. This is the main
    // difference from the old force cloud and prevents visual pile-ups.
    ...
    for (const ringNodes of chunks(depthNodes, maxPerRing)) {
      ...
      const chordRadius =
        count <= 1
          ? 0
          : minimumChord / (2 * Math.sin(Math.PI / count));
      const circumferenceRadius =
        ringNodes.reduce((total, node) => total + radiusFor(node) * 2 + spacing, 0) /
        (Math.PI * 2);
      ...
      // Alternate phase between rings so edges fan out rather than forming
      // visually heavy radial columns.
  ...
  // Pinned/manual positions are respected only when they do not collide with an
  // already placed circle. A manual drag can therefore persist without allowing
  // the graph to regress into an unreadable overlap pile.
  for (const node of graph.nodes) {
